...
/// 返回一条记录的crc
static uint32_t GetCrc32(RecordHeader& header, const Key& key,
                         const Value& value) {
  uint32_t crc{};
...
/// 设置header,返回一条记录的总大小
static size_t FillHeader(RecordHeader& header, const Key& key,
                         const Value& value) {
...
RecordInfo WriteRecord(const Key& key, const Value& value, FILE* fp) {
  ...
  LOG(TRACE) << "write record crc32: " << header.crc32_ << " key: `" << key
