...
std::shared_ptr<ActiveFile> ActiveFile::Restore(size_t file_id,
                                                const std::string_view filename,
                                                RecordMap& record_map,
                                                ActiveMap& map) {
  auto callback = [&record_map, &map, file_id](const Key& key,
                                               const Value& value,
                                               const RecordInfo& info) {
    map.Apply(key, value);
    record_map.Apply(key,
                     HintRecord{.file_id_ = file_id,
